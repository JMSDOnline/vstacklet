#!/bin/bash
#
# [VStacklet Varnish LEMP Stack Development Script]
#
...
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/vstacklet-ubuntu-stack.sh >/dev/null 2>&1;
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/files-backup.sh >/dev/null 2>&1;
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/database-backup.sh >/dev/null 2>&1;
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/package-backups.sh >/dev/null 2>&1;
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/backup-cleanup.sh >/dev/null 2>&1;
...
curl -LO https://raw.githubusercontent.com/JMSDOnline/vstacklet/development/vs-backup >/dev/null 2>&1;
chmod +x vs-backup
...
function _novstacklet() {
  if [[ ${vstacklet} == "no" ]]; then
    ...
    echo "${bold}${cyan}type${normal} ${green}${bold}./dev-vstacklet.sh${normal} - ${bold}${cyan}followed by tapping Enter on your keyboard.${normal}"
  fi
}
